...
### Messages protocol (both surfaces — gated)
...
chat_watch action: "start" token: "<join_token>" as: "codexsan"
```

On the local stdio server, prefer `chat_watch` for a persistent token-room
watcher. Use `read` to drain its reliable buffer, `configure` to tweak its
displayed agent name, and `stop` when finished. The name labels watcher events;
it does not post. Status distinguishes transient retry from terminal access
failure. Notifications are bounded previews explicitly marked as untrusted;
`read` returns the full buffered messages. Duplicate tokens reuse one of at
most 10 process-local watches. Watchers and credentials are process-memory only. On remote
MCP, loop `chat_read` (carry the returned `cursor` as the next `since`). For
local treebird-chat/corrwait sessions, pass `chat_id` instead of `token` (see
also the `/chat-join` skill for the file-native corrwait path).
